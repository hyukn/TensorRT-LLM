...
__global__ void computeFP8QuantizeScalePerTensorVec(
    T_S* quant_ptr, __nv_bfloat16 const* weights, int64_t const numel)
{
    constexpr float min_scaling_factor = 1.0f / (FP8_E4M3_MAX * 512.f);
    int64_t const vecElements = numel / kVecSize;
    int64_t const stride = static_cast<int64_t>(blockDim.x) * gridDim.x;
...
#pragma unroll
    ...
    int64_t const tailStart = vecElements * kVecSize;
    for (int64_t i = tailStart + threadIdx.x + static_cast<int64_t>(blockIdx.x) * blockDim.x; i < numel;
         i += stride)
    {
