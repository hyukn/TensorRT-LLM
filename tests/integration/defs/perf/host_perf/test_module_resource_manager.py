...
This complements the scheduler module test (which uses a mock KV cache manager)
by testing the actual C++ add_sequence/add_token/refresh_blocks operations that
the mock cannot exercise.

Scenarios:
  - Generation prepare_resources at BS=1/8/32/64 (steady-state decode path)
  - Context prepare_resources at BS=1/8 (new request allocation path)
...
# ---------------------------------------------------------------------------
# Test scenarios: generation path (steady-state decode)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    "scenario",
    GENERATION_SCENARIOS,
    ids=[s[0] for s in GENERATION_SCENARIOS],
)
def test_kv_cache_prepare_generation(kv_cache_manager, scenario):
    """Benchmark prepare_resources() for the generation (decode) path.
...
# ---------------------------------------------------------------------------
# Test scenarios: context path (new request allocation)
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    "scenario",
    CONTEXT_SCENARIOS,
    ids=[s[0] for s in CONTEXT_SCENARIOS],
)
def test_kv_cache_prepare_context(kv_cache_manager, scenario):
    """Benchmark the context (new request) allocation path.
